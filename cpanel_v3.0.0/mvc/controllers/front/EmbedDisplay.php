...
class EmbedDisplay {
    ...
    public static function get_embed_info(){
        ...
        try {
            ...
            $embed_id = ( $embed_id  <= 0 ) ? -1 : $embed_id;

            if( CACHING_ENABLED ){
                $cache  = Cache::create_instance("embed.$embed_id.cache.json");
            ...
            //$embed_path = SectionDB::get_section_path($parent_id);
            $embed_path   = SectionTreeJSON::get_section_path($parent_id);
            $embed_path[] = $embed;

            if( isset($embed) ){
                $layout->title  = Dictionary::get_text_by_lang($embed, "title") . ' | ' . $layout->title_postfix;
                $layout->tags[] = array( "name" => "keywords"   , "content" => Dictionary::get_text_by_lang($embed, "keys") );
                $layout->tags[] = array( "name" => "description", "content" => Dictionary::get_text_by_lang($embed, "desc") );
            }
            ...
            $frame_data["lang_ar"] = UrlUtil::get_section_child_href($embed, "ar");
            $frame_data["lang_en"] = UrlUtil::get_section_child_href($embed, "en");

            $frame_data["frame"] = $layout;
            $frame_data["lang"]  = $lang;

            $front_tpl = TplLoader::get_tpl_data('embed_info.tpl.php', 'mvc/views/pages', $page_data);
