...
class AuthManage extends ManageAjax {
    ...
    public static function logout(){
        ...
        try {
            ...
            $session->remove_attribute("admin");
    ...
    public static function check_exist(){
        ...
        try {
            ...
            $admin_options["email"]    = $entry;
            $admin_options["mobile"]   = $entry;
            //$admin_options["active"] = 1;
            
            $admin  = UserDB::get_user_with_one_of($admin_options);
            
            if( $admin != null && $admin->admin_id > 0 ){
    ...
    public static function register_action(){
        ...
        try {
            ...
            if( $status > 0){
                ...
                $admin_key = md5( KEY_GEN . '' . time() );

                //$admin_arr['fname']      = $request->get_parameter("fname");
                //$admin_arr['lname']      = $request->get_parameter("lname");
                $admin_arr['name']       = $request->get_parameter("name");
                $admin_arr['password']   = $password;
                $admin_arr['email']      = $request->get_parameter("email");
                $admin_arr['phone']      = MobileUtil::remove_dial_code( $request->get_parameter("phone") );
                $admin_arr['country']    = $request->get_parameter("country");
                $admin_arr['key']        = $admin_key;
                $admin_arr['code']       = '';
                $admin_arr['icon']       = '';
                $admin_arr['birth_date'] = $request->get_parameter("birth_date");
                $admin_arr['gender']     = $request->get_parameter("gender");

                $create_date = date(self::$date_format);

                $admin_arr['created']      = $create_date;
                $admin_arr['updated']      = $create_date;
                $admin_arr['suspend_date'] = null;
                
                $admin_arr['status']     = USER_STATUS_NOT_VERIFIED;
                $admin_arr['rule_id']    = USER_RULE_NORMAL;
                $admin_arr['options']    = 0;

                $admin = (object) $admin_arr;
                
                $status = UserDB::add_user($admin);
            ...
            if( $status > 0){
                ...
                $verify_link = UrlUtil::get_verify_href($admin_id, $admin_key);
    ...
    public static function forget_password_action() {
        ...
        try {
            ...
            if( $status > 0){
                ...
                $admin = UserDB::get_user_with_one_of($options);
                ...
                if( !empty($admin) ){
                    ...
                    $reset_link = UrlUtil::get_reset_href($admin_id, $admin_key);
    ...
    public static function reset_password_action() {
        ...
        try {
            ...
            if( !empty($password) && $password == $password_confirm ){
                ...
                $admin_key = $request->get_parameter("user_key");
                
                $admin = UserDB::get_user($admin_id);
                ...
                if( !empty($admin) ){
                    ...
                    if( $admin_key == $admin->key){
                        ...
                        $status = UserDB::update_user($admin);
