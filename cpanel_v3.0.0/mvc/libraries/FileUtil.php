...
class FileUtil {
    ...
    public static function resize_image_keep_aspect($image_name, $image_dir, $bound_width, $bound_height) {
        ...
        try{
            ...
            //$ext = substr($imageFile, strpos($imageFile, "."), strlen($imageFile) );
            $ext = substr($image_file, strripos($image_file, "."), strlen($image_file) );
            ...
            if (preg_match("/jpg|jpeg/",$ext)){
                ...
                $src_img = imagecreatefromgif($image_file);
            }
            ...
            if ($new_width < $bound_width) {
                $dst_x = ( $bound_width  - $new_width  ) / 2;
            }
            if ($new_height < $bound_height) {
                $dst_y = ( $bound_height - $new_height ) / 2;
            }
            ...
            //Create Tranparent BG
            $bg_img = imagecreatetruecolor($bound_width, $bound_height);

            imagesavealpha($bg_img, true);

            $color = imagecolorallocatealpha($bg_img, 0, 0, 0, 127);

            imagefill($bg_img, 0, 0, $color);
            
            imagecopy($bg_img, $dst_img, $dst_x, $dst_y, 0, 0, imagesx($dst_img), imagesy($dst_img));

            
            imagepng($bg_img, $image_file);
    ...
    public static function resize_image_keep_aspect_add_watermark($image_name, $image_dir, $bound_width, $bound_height, $stamp_file) {

        try{

            $image_file = $image_dir.'/'.$image_name;

            $src_img = self::create_image_resource($image_file);

            $ext     = self::get_image_type($image_file);
            ...
            ////////////////////////////////////////////////////////////////////
            //| Centerize Image & Add Transparent BG |//////////////////////////
            ////////////////////////////////////////////////////////////////////
            ...
            // width to calculate positioning of the stamp. 
            //imagecopy($image, $stamp, $marge_right, imagesy($image) - $sy - $marge_bottom, 0, 0, imagesx($stamp), imagesy($stamp));
            imagecopy($bg_img, $stamp, imagesx($bg_img) - $stamp_x - $marge_right, imagesy($bg_img) - $stamp_y - $marge_bottom, 0, 0, imagesx($stamp), imagesy($stamp));
    ...
    public static function create_thumbnail($imageName, $thumbName, $newW, $newH, $imageDir, $thumbDir=null) {
        ...
        try{
            ...
            $srcImg = self::create_image_resource($imageFile);

            $ext    = self::get_image_type($imageFile);
...
//            }
    ...
    public static function replace_thumb($imageName, $thumbPrefix, $newW, $newH, $oldThumb, $imageDir, $thumbDir=null){
        ...
        try{
            ...
            $no_change = ($uploadedFile == null) || ( $uploadedFile->size <= 0 );
    ...
    public static function create_image_resource($image_file){
        ...
        try{
            ...
            throw new CustomException( 'Error in : create image resource', $e );//from php 5.3 no need to custum
        }
    ...
    public static function get_image_type($image_file){
