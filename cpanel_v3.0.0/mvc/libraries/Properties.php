...
/**
 * @ignore Create "Standard PHP Library" compatible CustomException classes if the SPL extension isn't available.
 * @ignore The SPL extension exists in PHP 5.0, but is only installed by default since PHP 5.1.
 */
//if (!extension_loaded('SPL')) { 
  if (!class_exists('LogicCustomException')) {
    eval('class LogicCustomException extends CustomException {};');
  }
  if (!class_exists('InvalidArgumentCustomException')) {
    eval('class InvalidArgumentCustomException extends LogicCustomException {};');
  }
//}
 ...
 * It makes use of CustomException classes from the "Standard PHP Library" extension (http://www.php.net/spl)
...
class Properties {
   ...
   * @throw InvalidArgumentCustomException
   */
  ...
  public function __construct($defaults = null) { // This is for PHP 5.0 up.
    if (isset($defaults)) {
      if (!($defaults instanceof Properties)) {
    	throw new InvalidArgumentCustomException('The $defaults parameter must be null or an instance of ' . __CLASS__ . '.');
      }
  ...
  /**
   * Tests an unescaped key's syntax. Throws an CustomException on error.
   *
   * @param key   
   * @throw InvalidArgumentCustomException if key syntax is invalid.
   */
  protected static function _testKey($key) {
    if (!(isset($key) && strlen($key))) {
      throw new InvalidArgumentCustomException('The $key parameter must be a string of 1 or more characters.');
    }
  }
  ...
  public function load($source) {
    if (!isset($source)) {
      throw new InvalidArgumentCustomException('The $source parameter may not be null.');
    }
    $lines = null;
    if (is_array($source)) {
      $lines = $source;
    ...
    elseif (get_resource_type($source) === 'file') {
      ...
      throw new InvalidArgumentCustomException('The $source parameter of type "' . gettype($source) . '" is not supported.');
    }
    ...
    if (array_key_exists($key, $this->properties)) {
      ...
      for ($i = 0; $i < count($this->sections); $i++) {
        if (($this->sections[$i] instanceof Properties_Section_Property) && ($this->sections[$i]->getKey() == $key)) {
          ...
          if ($i > 0) {
            $previous_section_division = get_division($this->sections[$i - 1]);
            if ($previous_section_division != 'Properties_Section_Property') {
              for ($j = $i - 1; $j >= 0; $j--) {
                if (get_division($this->sections[$j]) == $previous_section_division) {
                  $splice_from = $j;
                }
                else {
                  break;
      ...
      if (!$section_removed) {
      	throw new LogicCustomException("Cannot find (and therefore remove) the section with key '$key'.");
      }
      // remove the property
...
class Properties_Section_Comment implements Properties_ISection {
  ...
  protected static function _testCommentChar($char) {
    if (!isset($char)) {
      throw new InvalidArgumentCustomException('Comment character may not be null.');
    }
    if (($char != '#') && ($char != '!')) {
      throw new InvalidArgumentCustomException('String "' . $char . '" is not a valid comment character.');
    }
  }
  ...
  protected static function _testPadding($string) {
    if (!isset($string)) {
      throw new InvalidArgumentCustomException("Padding may not be null!");
    }
    if (!preg_match('/^( |\\t|#|!)*$/', $string)) {
      throw new InvalidArgumentCustomException('Padding string contains invalid characters.');
    }
  }
...
class Properties_Section_Property implements Properties_ISection {
  ...
  public function __construct() {
    ...
    case 1:
      ...
      if (!(isset($line) && strlen($line))) {
      	throw new InvalidArgumentCustomException('Empty line passed into overloaded constructor.');
      }
      $parts = $this->_parseLine($line);
      if (!$parts) {
        throw new InvalidArgumentCustomException('Invalid property line passed into overloaded constructor.');
      }
    ...
    case 3:
      ...
      break;
    default:
      throw new InvalidArgumentCustomException('Invalid arguments passed into overloaded constructor.');
    }
    $this->_testKey($key);
  ...
  protected static function _testKey($key) {
    if (!isset($key)) {
      throw new InvalidArgumentCustomException("Property key may not be null!");
    }
    if (!strlen($key)) {
      throw new InvalidArgumentCustomException("Property key may not be empty!");
    }
  }


  /**
   * Tests an unescaped value's syntax. Throws an CustomException on error.
   *
   * @param value
   * @throw InvalidArgumentCustomException
   */
  protected static function _testValue($value) {
    if (!isset($value)) {
      throw new InvalidArgumentCustomException("Property value may not be null!");
    }
  }


  /**
   * Tests a seperator's syntax. Throws an CustomException on error.
   *
   * @param seperator   
   * @throw InvalidArgumentCustomException
   */
  protected static function _testSeperator($seperator) {
    if (!isset($seperator)) {
      throw new InvalidArgumentCustomException("Seperator may not be null!");
    }
    if (!strlen($seperator)) {
      throw new InvalidArgumentCustomException("Seperator may not be empty!");
    }
    if (!preg_match('/^' . self::SEPERATOR_REGEX_PATTERN . '$/', $seperator)) {
      throw new InvalidArgumentCustomException("Bad syntax in seperator string!");
    }
  }
  ...
  protected static function _unescapeChar($s) {
    ...
    if (strlen($s) > 1) {
      if (!preg_match('/^u[\da-fA-F]{4}$/', $s)) {
        throw new CustomException("Malformed \\uxxxx encoding in '\\$s'.");
      }
